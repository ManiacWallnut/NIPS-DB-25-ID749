...
class Task:
    ...
    def auto_done_task(self, scene_graph_tree):
        pass
    ...
    def __repr__(self):
        ...
        noslash_name = StringConvertor.get_noslash_name(self.item.name)
        noslash_destination_name = self.destination.get_name_for_interaction()
        if self.feature and len(self.feature) > 0:
            noslash_feature0_name = StringConvertor.get_noslash_name_wo_id(
                self.feature[0]
            )
        if self.feature and len(self.feature) > 1:
            noslash_feature1_name = StringConvertor.get_noslash_name_wo_id(
                self.feature[1]
            )
            
        if self.type == TaskType.MOVE_TO_EMPTY_PLATFORM:
            return f"Move {noslash_name} to {noslash_destination_name}.\n\n"
        elif self.type == TaskType.MOVE_TO_EMPTY_PLATFORM_9_GRID:
            return f"Move {noslash_name} to {noslash_destination_name}'s {self.feature[0]} part\n\n"
        elif self.type == TaskType.MOVE_AROUND_OBJECT:
            return f"Move {noslash_name} around {noslash_feature0_name} \n\n"
        elif self.type == TaskType.MOVE_TO_OBJECT_FREESPACE_9_GRID:
            return f"Move {noslash_name} to {noslash_feature0_name}'s {noslash_feature1_name} freespace\n\n"
        elif self.type == TaskType.MOVE_TO_MIDDLE_OF_OBJECTS:
            return f"Move {noslash_name} between {noslash_feature0_name} and {noslash_feature1_name} \n\n"
    ...
    def generate_goal_information(
        ...
        goto_destination_platform_explanation = (
            f"Now you have picked up object, you can see the scene {image_name_list}. Go to {destination_platform_name} to place {source_item_name}."
        )
        ...
        # elif self.type == TaskType.MOVE_TO_MIDDLE_OF_OBJECTS:
        #     object_to_show_a = scene_graph_tree.nodes[self.feature[0]]
        #     object_to_show_b = scene_graph_tree.nodes[self.feature[1]]
        #     img_a = object_to_show_a.auto_take_non_ground_object_picture(
        #         scene=scene_graph_tree.corresponding_scene,
        #         view="human_focus",
        #         mark_object=False,
        #         only_mark_itself=False,
        ...
        #     ]
        #     img_b = object_to_show_b.auto_take_non_ground_object_picture(
        #         scene=scene_graph_tree.corresponding_scene,
        #         view="human_focus",
        #         mark_object=False,
        #         only_mark_itself=False,
        ...
        if self.type == TaskType.MOVE_TO_EMPTY_PLATFORM:
            ...
            for i in range(4):
                ...
                platform_img, platform_img_list = (
                    scene_graph_tree.auto_take_platform_picture(
                        ...
                        mark_object=True,
                        mark_freespace=len(self.destination.children) == 0,
                        standing_direction=available_directions[first_standing_direction_id],
                        width=width,
                        height=height,
                        focus_ratio=0.6,
            ...
            freespace_list = [(select_item_id_a, dir) for dir in ba_dir if dir != -1] + [
        ...
        for item_node in item_node_list:
            ...
            for place_node in place_node_list:
                platform_list = [
                    ...
                    if len(
                        [
                            place_node.freespace_is_standable(dir)
                            for dir in range(0, 8, 2)
                        ]
                    )
                    and (platform.avl_height > 0.01 or len(platform.children) > 0)
                ]
                
                glog.info([platform.name for platform in platform_list])
                for place_platform in platform_list:
                    if (
                        "sofa_10" in place_platform.name
                        or "table_04_13" in place_platform.name
                        and place_platform.name[-1] == "0"
                    ):
                        continue
                    ...
                    available_directions = [
                        dir
                        for dir in range(0, 8, 2)
                        if place_node.freespace_is_standable(dir)
                    ]
                    if len(available_directions) == 0:
                        continue
                    available_directions = available_directions[:1]
