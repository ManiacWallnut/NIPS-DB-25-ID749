...
from custom_geometry_helper_new.convex_hull_processor import (
    ConvexHullProcessor_2d,
)
from custom_geometry_helper_new.basic_geometries import Basic2DGeometry
from custom_geometry_helper_new.polygon_processor import PolygonProcessor
...
from custom_string_parse_helper.string_convertor import StringConvertor
import sapien
import transforms3d
...
class TreePlatform:
    ...
    def get_categories_and_objects(self):
        ...
        children_name_list = [child.get_name_for_interaction() for child in self.children]
...
class TreeNode:
    ...
    def __init__(
        ...
        self.object = parse_objects.SceneElement(
            name=name,
            heading=heading,
    ...
    def is_multilayer_object(self):
        ...
        real_layers = 0
        for platform in self.own_platform:
            real_layers += len([dir for dir in range(0, 8, 2) if self.freespace_is_standable(dir) and platform.freespace_is_visible(dir)]) > 0
    ...
    def get_categories_and_objects_for_mlo(self):
        ...
        minor_category = set()
        for platform in self.own_platform:
            major_category_list, minor_category_list, _ = platform.get_categories_and_objects()
    ...
    def reset_diagonal_accurate_space(self):
        for straight_part in range(0, 8, 2):
            self.free_space[straight_part]["Accurate_space"] = self.free_space[
                straight_part
            ]["Available_space"]
        self.object.bbox = self.object.convex_hull_2d.get_headed_bbox_instance()
        for diagonal_part in range(4):
            i = 2 * diagonal_part + 1
            import copy

            self.free_space[i]["Accurate_space"][(diagonal_part + 3) % 4] = (
                copy.deepcopy(
                    self.free_space[(i + 7) % 8]["Accurate_space"][diagonal_part]
                )
            )
            self.free_space[i]["Accurate_space"][(diagonal_part + 2) % 4] = (
                self.object.bbox[diagonal_part]
            )
            self.free_space[i]["Accurate_space"][(diagonal_part + 1) % 4] = (
                self.free_space[(i + 1) % 8]["Accurate_space"][diagonal_part]
            )
            self.free_space[i]["Accurate_space"][diagonal_part] = (
                self.free_space[i]["Accurate_space"][(diagonal_part + 3) % 4]
                + self.free_space[i]["Accurate_space"][(diagonal_part + 1) % 4]
                - self.free_space[i]["Accurate_space"][(diagonal_part + 2) % 4]
            )
    ...
    def auto_take_non_ground_object_picture(
        ...
        for dir in range(0, 8, 2):
            if bel_ground_node.freespace_is_standable(
                standing_direction
            ) and bel_ground_platform.freespace_is_visible(standing_direction):

                break
            standing_direction = (standing_direction + 2) % 8

        self_center = self.get_center()
        border = bel_ground_node.get_bbox_line(standing_direction)
    ...
    def auto_take_ground_object_picture(
        ...
        if self.depth != 1:
            print("This function is only for ground object")
            return None
        ...
        camera_xy = (
            self.get_standing_point(direction=standing_direction)
            if "top" not in view
            else self.get_center()
        )
        ...
        pose_2d_list = self.get_multiple_standing_point(
            platform_rect=platform_rect, standing_direction=standing_direction
        )
...
class Tree:
        ...
        for ground_object in ground_object_list:
            for platform in ground_object.own_platform:
                if (len(platform.children) > 0 or platform.avl_height > 0.1) and any(
                    [
                        ground_object.freespace_is_standable(dir)
    ...
    def from_scene_platform_list(self, object_platform_list, contacts):
        ...
        # self.nodes[item.belong].own_platform
        """
        def __init__(
        self,
        name='',
        ...
        glog.info("platforms: %s", self.platforms.keys())
        glog.info(len(contacts))
    ...
    def remove_node(self, name):
        ...
        if node is not None:
            ...
            for child in node.children:
                ...
                node.on_platform.children.append(child)
...
def gen_scene_graph_for_sunrgbd(json_data):
    ...
    object_list = [parse_objects.SceneObject.from_box(
        name=obj["template_name"],
        centroid_translation=obj["centroid_translation"],
        quaternion=obj["quaternion"],
        dimension=obj["non_uniform_scale"],
    ) for obj in all_objects]
    
    object_list.append(ground)
    
    scene_platform_list.append(
        parse_objects.SceneElement(
            name="GROUND_0",
            heading=(1, 0),
            height=0,
            avl_height=3.08,
    ...
    sorted_scene_platform_list, contacts_id = (
        parse_objects.SceneElement.calculate_contact_conditions(scene_platform_list)
    )
