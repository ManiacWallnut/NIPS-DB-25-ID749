...
class VLMInteractor:
    ...
    MAX_INTERACTION_COUNT = 40

    def __init__(self, mode="debug", model="GPT4o", manual_list = []):
        ...
        self.manual_list_index = 0
        for i, str in enumerate(self.manual_list):
            str = str.replace("_(success)", "")
            str = str.replace("_(failure)", "")
            str = str.replace("pickup", "pick_up")
            str = str.replace("place_occupied_platform", "place_at_freespace")
            str = str.replace("place_empty_platform", "place_at_freespace")
            str = str.replace("place_anywhere", "place_at_anywhere")
            self.manual_list[i] = str
        pass
    ...
    def send_content_n_request(self):
        ...
        if self.mode == "online":
            ...
            return InteractStatusCode.SUCCESS, answer1
        elif self.mode == 'manual_list':
            if self.manual_list_index >= len(self.manual_list):
                return InteractStatusCode.SUCCESS, 'CALL_END'
            result = self.manual_list[self.manual_list_index]
            self.manual_list_index += 1
            return InteractStatusCode.SUCCESS, result
