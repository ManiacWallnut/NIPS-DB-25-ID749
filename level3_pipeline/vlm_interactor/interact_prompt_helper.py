...
class ImagePromptFormatter(PromptFormatter):
    ...
    def new_format_prompt(
        ...
        Args:
            variant_type: 变体类型 (如 'identical_object_to_move')
            image_info_list: 图像信息列表
            shared_vars: 所有图像共享的变量
        """
        if variant_type not in self.template.variants:
            raise ValueError(f"未知的变体类型: {variant_type}")
        ...
        if shared_vars and variant.shared_variables:
            ...
            if missing_vars:
                raise ValueError(f"缺少共享变量: {missing_vars}")
        ...
        for img_info in image_info_list:
            ...
            if missing_vars:
                raise ValueError(f"图像信息缺少必需变量: {missing_vars}")
    ...
    def __init__(
        ...
        try:
            with open(prompt_templates, "r", encoding="utf-8") as f:
                self.prompt_templates = json.load(f)  # 使用load而不是loads
            self.state_config = self.prompt_templates["states"]
        except FileNotFoundError:
            glog.error(f"找不到提示模板文件: {prompt_templates}")
            raise
        except json.JSONDecodeError as e:
            glog.error(f"JSON格式错误: {e}")
            raise
        except KeyError as e:
            glog.error(f"提示模板中缺少必要的键: {e}")
            raise
...
class HintPromptManager:
    ...
    def __init__(self, prompt_templates):
        ...
        try:
            ...
            glog.error(f"找不到提示模板文件: {prompt_templates_path}")
            raise
        except json.JSONDecodeError as e:
            glog.error(f"JSON格式错误: {e}")
            raise
