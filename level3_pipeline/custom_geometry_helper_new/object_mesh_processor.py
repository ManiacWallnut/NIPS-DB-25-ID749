...
from . import convex_hull_processor, basic_geometries
import glog
...
class AffordablePlatform:
    ...
    def block_by_other_platform(self, other_platform, threshold=0.5):
        ...
        if (
            ...
            or intersection_polygon.get_area() < MeshProcessor.EPS
        ):
            return False
...
class MeshProcessor:
    ...
    def get_raw_affordable_platforms(
        ...
        if not inverse:
            ...
            self.inverse_affordable_platforms = separate_face_list
    ...
    def calculate_affordable_platforms(self, raw_platform=True, top_area=0):
        ...
        for i, bottom_platform in enumerate(self.affordable_platforms):
            ...
            for j, other_platform in enumerate(self.affordable_platforms):
                ...
                if (
                    other_height < bottom_height - MeshProcessor.EPS
                    or other_height
                ...
                if (
                    ...
                    or intersection_convex.get_area() < MeshProcessor.EPS
                ):
                    continue
                if area_bottom * 0.6 < intersection_convex.get_area():
            ...
            for j, other_platform in enumerate(self.inverse_affordable_platforms):
                ...
                # if top_height - other_height < MeshProcessor.EPS:
                #     continue
                # intersected_polygon = PolygonProcessor.intersect_two_polygons(
                #     top_polygon, other_polygon
                # )
                # intersected_polygon = PolygonProcessor(intersected_polygon)
                # intersected_pieces = intersected_polygon.get_len()
                # intersected_area = intersected_polygon.get_area()

                # top_polygon_area = PolygonProcessor(other_polygon).get_area()

                # if intersected_pieces == 0 or intersected_area < MeshProcessor.EPS:
                #     continue
        ...
        # self.cal_platform_avl_height()
        # self.clear_too_low_platforms()
    ...
    def clear_too_low_platforms(self, height_threshold=0.01):
        if len(self.affordable_platforms) == 0:
            return self
        self.affordable_platforms = [
            platform
    ...
    def check_platform_visability(
        ...
        """
        observation_positions = {
            "right": np.array([[(x_min + x_max) / 2, (y_min + y_max) / 2, height_range[0] + i * (height_range[1] - height_range[0]) / height_steps] for i in range(height_steps+1)]),
            "rear": np.array([[(x_min + x_max) / 2, (y_min + y_max) / 2,  height_range[0] + i * (height_range[1] - height_range[0]) / height_steps] for i in range(height_steps+1)]),
            "left": np.array([[(x_min + x_max) / 2, (y_min + y_max) / 2, height_range[0] + i * (height_range[1] - height_range[0]) / height_steps] for i in range(height_steps+1)]),
            "front": np.array([[(x_min + x_max) / 2, (y_min + y_max) / 2,  height_range[0] + i * (height_range[1] - height_range[0]) / height_steps] for i in range(height_steps+1)]),
        }
        """
                    ...
                    for i, point in enumerate(sample_platform_point_list):
                        ...
                        for triangle in mesh_triangles:
                            ...
                            if intersection is not None:
                                ...
                                if int_distance < view_distance * 0.92:
                                    hit = True
                                    break
                        if not hit:
                            visible_cnt += 1
