...
def main():
    ...
    parser.add_argument("--json_file_path", type=str, default="replica_apt_0.json")
    parser.add_argument("--output_pose_path", type=str, default="entities_apt_0.json")
...
if __name__ == "__main__":
    main()
#%% wide view_pose_adjustment
'''
pose_a
    camera = add_camera(name="", width=1920, height=1080, fovy=2.27, near=0.05, far=100)
    camera.set_local_pose(Pose([3.86561, 0.848194, 2.91362], [0.462053, 0.107553, 0.0565702, -0.878487]))
pose_b
    camera = add_camera(name="", width=1920, height=1080, fovy=2.09, near=0.05, far=100)
    camera.set_local_pose(Pose([4.23576, -7.79106, 2.82348], [0.460129, -0.223425, 0.12084, 0.850741]))
pose_c
    camera = add_camera(name="", width=1920, height=1080, fovy=2.5, near=0.05, far=100)
    camera.set_local_pose(Pose([4.19473, -5.22497, 3.54974], [0.0867526, -0.226415, 0.0202503, 0.969949]))


put lamp on the tvstand
camera = add_camera(name="", width=1920, height=1080, fovy=2.23, near=0.05, far=100)
camera.set_local_pose(Pose([2.86925, -1.68342, 2.42448], [0.535007, 0.0289851, 0.018371, -0.84415]))
'''