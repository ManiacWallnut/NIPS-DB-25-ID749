...
level3_task_list = [ (wanted_task_list[i], random.choice(wanted_task_list)) for i in range(0, len(wanted_task_list), 17)]
...
random.seed(2025)
level3_task_list = [ (i, random.randint(0, 44)) for i in range(0, len(wanted_task_list))]

level3_task_list = [(i, j) for i, j in level3_task_list if i != j]
level3_task_list = random.sample(level3_task_list, 16)
level3_task_list.pop(11)
...
for i, j in (level3_task_list):
    ...
    task_a = task_list[i]
    task_b = task_list[j]
    task_a.generate_default_intermediate_state()
    task_b.generate_default_intermediate_state()
    ...
    with open(result_file_path, "a") as f:
        f.write(f"Task {i}: {task.status and task.intermediate_partial_score==4},  Score: {task.intermediate_partial_score} {task.partial_score}\n")
