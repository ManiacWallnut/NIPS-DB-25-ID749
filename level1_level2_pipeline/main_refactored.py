        ...
        for node in scene_graph_tree.nodes.values():
            ...
            if node.depth == 1:
                ...
                for platform_id, platform in enumerate(node.own_platform):
                    standing_direction = side
                    for dir in range(4):
                        if node.freespace_is_standable(standing_direction) == False or platform.freespace_is_visible(standing_direction):
                            standing_direction = (standing_direction + 2) % 8
                        else:
                            break
                        
                    img = platform_img, platform_img_list = (
...
task_sample_file_pkl = "task_sample.pkl"
...
task_list = wrong_task_id_train_list if args.mode == 'generate_reflection' else full_id_list
print(len(task_list))
