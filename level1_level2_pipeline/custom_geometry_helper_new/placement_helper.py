...
class PlacementHelper:
    ...
    def place_rectangle_anywhere(fixed_rect_list, moving_rect, bound_rect):
        ...
        for x in x_coords:
            if not x_coords_new:
                ...
                if np.abs(x - x_coords_new[-1]) > 1e-6:
                    x_coords_new.append(x)
        ...
        for y in y_coords:
            if not y_coords_new:
                ...
                if np.abs(y - y_coords_new[-1]) > 1e-6:
                    y_coords_new.append(y)
    ...
    def find_parallel_rectangle_placement_bounds(fixed_rect, moving_rect):
        ...
        if np.abs(normalized_angle - _) > 1e-3:
    ...
    def find_placement_bounds_contained_by_rectangle_union(
        ...
        #        glog.info(f"normalized_fixed_rect_list: {normalized_fixed_rect_list}")
        #        glog.info(f"all_intersected: {all_intersected}")
    ...
    def place_rectangle(fixed_rect_list, moving_rect, need_all_intersect=True):
        """
        Place the rectangle in the area where it intersects with all the rectangles
        """
