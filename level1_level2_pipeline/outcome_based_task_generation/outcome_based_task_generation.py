...
class OBTPlatform:
    def __init__(self, platform_index: int, platform_name: str = None,
    ...
    def is_available(self, major_num=0, minor_num=0, single_num=0):
        ...
        """
        return len(self.major_category_list) >= major_num and \
               len(self.minor_category_list) >= minor_num and \
               len(self.single_object_list) >= single_num
...
class OBTMultilayerObject:
    def __init__(self, object_index: int, multilayer_object_name: str = None, 
                 major_category_list: list = [], 
                 minor_category_list: list = [],
                 layer_num: int = 0):
        """
    ...
    def is_available(self, major_num=0, minor_num=0):
        ...
        """
        return len(self.major_category_list) >= major_num and \
               len(self.minor_category_list) >= minor_num
    ...
    def random_pick(self, requirement:dict):
        ...
        if 'requires_top_layer' in requirement and requirement['requires_top_layer']:
            ...
            pass
        if 'requires_specific_layer' in requirement and requirement['requires_specific_layer']:
            # Randomly pick from the specific layer
            specific_layer_name = self.multilayer_object_name + f"_{random.randint(self.layer_num - 1)}_layer"
            pass
        if 'category_objects_count' in requirement and requirement['category_objects_count'] > 0:
            category_name_list = random.sample(self.major_category_list, requirement['category_objects_count'])
        if 'small_category_objects_count' in requirement and requirement['small_category_objects_count'] > 0:
            small_category_name_list = random.sample(self.minor_category_list, requirement['small_category_objects_count'])
        ...
        return top_layer_name, specific_layer_name, category_name_list, small_category_name_list
        pass
...
class OutcomeBasedTaskPattern:
    def __init__(self, task_pattern: str):
        ...
        self.require_platform_list = OutcomeBasedTaskPattern.parse_platform_requirements(self.task_pattern)
        self.require_multilayer_object_info = OutcomeBasedTaskPattern.parse_multilayer_object_requirements(self.task_pattern)
        self.require_room_objects = OutcomeBasedTaskPattern.parse_room_object_requirements(self.task_pattern)
        
        self.pattern_info_dict = {}
        
        if self.require_multilayer_object_info is not None:
            self.task_base_type = "multi-layer-object"
        elif self.require_platform_list: # If not multi-layer but has platform requirements
            self.task_base_type = "platforms"
        elif self.require_room_objects["large_category_objects_count"] > 0 or \
             self.require_room_objects["small_category_objects_count"] > 0:
    ...
    def generate_task_description(self, platform_list, multilayer_object_list, room_object_list):
        ...
        multilayer_object_name_list = [multilayer_object.multilayer_object_name for multilayer_object in multilayer_object_list]
        multilayer_feature = multilayer_object_list[0].random_pick(self.require_multilayer_object_info) if len(multilayer_object_list) > 0 else None
        room_major_category_list = [room_major_category.major_category for room_major_category in room_object_list]
        room_minor_category_list = [room_minor_category.minor_category for room_minor_category in room_object_list]
        
        for id, platform in enumerate(platform_list):
            requirement = self.require_platform_list[id]
            major, minor, single = platform.random_pick(requirement[0], requirement[1], requirement[2])
        ...
        def replace_placeholder(match):
            ...
            if placeholder.startswith("[PLATFORM"):
                ...
                if platform_index < len(platform_list) and object_index < len(platform_list[platform_index].major_category_list):
                    return platform_list[platform_index].major_category_list[object_index]
                else:
                ...
                if platform_index < len(platform_list) and object_index < len(platform_list[platform_index].minor_category_list):
                    return platform_list[platform_index].minor_category_list[object_index]
                else:
                ...
                if platform_index < len(platform_list) and object_index < len(platform_list[platform_index].single_object_list):
                    return platform_list[platform_index].single_object_list[object_index]
                else:
    ...
    def generate_task_images(self, scene_graph_tree, 
                             platform_name_list, 
        ...
        for id, platform in enumerate(platform_name_list):
            platform = None
            for platform_in_tree_name, platform_in_tree in scene_graph_tree.platforms.items():
                if platform_in_tree.get_name_for_interaction() == self.platform_name_list[id]:
        ...
        for id, multilayer_object in enumerate(multilayer_object_list):
            ...
            for multilayer_object_in_tree_name, multilayer_object_in_tree in scene_graph_tree.multilayer_objects.items():
                if multilayer_object_in_tree.get_name_for_interaction() == self.multilayer_object_name_list[id]:
    ...
    def parse_platform_requirements(pattern_string: str) -> list:
        """
        解析任务描述字符串，提取平台及其物体需求。
        相同的物体模板在同一平台只计数一次。
        如果物体pattern引用的平台未显式声明，则会记录警告。
        """
        platform_details = {} 
        ...
        category_matches = re.findall(r"\[SUB-PLATFORM-CATEGORY-OBJECT(\d+)(\d+)\]", pattern_string)
        ...
        small_category_matches = re.findall(r"\[SUB-PLATFORM-OBJECT(\d+)(\d+)\]", pattern_string)
        ...
        single_object_matches = re.findall(r"\[SUB-PLATFORM-SINGLE-OBJECT(\d+)(\d+)\]", pattern_string)
        ...
        if max_seen_platform_index == -1:
            if not (platform_direct_mentions or category_matches or small_category_matches or single_object_matches):
                 return []
            return []
    ...
    def parse_multilayer_object_requirements(pattern_string: str) -> dict:
        ...
        category_object_indices = set(re.findall(r"\[SUB-LAYER-CATEGORY-OBJECT(\d+)\]", pattern_string))
        category_objects_count = len(category_object_indices)

        small_object_indices = set(re.findall(r"\[SUB-LAYER-OBJECT(\d+)\]", pattern_string))
    ...
    def parse_room_object_requirements(pattern_string: str) -> dict:
        """
        解析字符串，提取对来自整个房间的物体的需求。
        """
        # Regex based on pattern.txt (e.g. SUB-OBJECT-CATEGORY0 - no 'S')
        large_category_indices = set(re.findall(r"\[SUB-OBJECT-CATEGORY(\d+)\]", pattern_string))
...
class OutcomeBasedTaskGenerator:
            ...
            with open(task_pattern_file, 'r', encoding='utf-8') as file:
                for line in file:
                    ...
                    if task_pattern:  # Ignore empty lines
                        self.task_pattern_list.append(OutcomeBasedTaskPattern(task_pattern))
    ...
    def generate_all_possible_tasks(self):
        ...
        if not self.platform_list or not self.multilayer_object_list:
            glog.error("Platform list or multilayer object list is empty. Cannot generate tasks.")
            return all_tasks
    ...
    def generate_sample_task(self):
        ...
        for task_pattern in self.task_pattern_list:
            ...
            required_multilayer_object_list = task_pattern.require_multilayer_object_info
            if len(required_platform_list) != 1:
                continue
            junior_platform_combinations = combinations(platform_list, len(required_platform_list) - 1)
            ...
            for senior_platform in well_occupied_platform_list:
                # import ipdb 
                # ipdb.set_trace()
                if senior_platform.is_available(task_pattern.require_platform_list[0][0], task_pattern.require_platform_list[0][1], task_pattern.require_platform_list[0][2]):
                        ...
                        if flag:
                            if required_multilayer_object_list is not None and len(required_multilayer_object_list) > 0:
                                for multilayer_object in multilayer_object_list:
                                    if not task_pattern.require_multilayer_object_info or multilayer_object.is_available(task_pattern.require_multilayer_object_info['category_objects_count'], task_pattern.require_multilayer_object_info['small_category_objects_count']):
                                        # Generate a task using the selected platforms

                                        # Generate a task using the selected platforms
                                        
                                        task_description = task_pattern.generate_task_description(platform_list = [senior_platform] , multilayer_object_list=[multilayer_object],room_object_list=[])
                                ...
                                # Generate a task using the selected platforms
                                task_description = task_pattern.generate_task_description(platform_list = [senior_platform], multilayer_object_list=[],room_object_list=[])
    ...
    def parse_scene_graph_tree(self):
        ...
        major_category_in_tree_list, minor_category_in_tree_list = self.scene_graph_tree.get_node_categories_in_scene()
        
        for id, major_category in enumerate(major_category_in_tree_list):
            room_major_category_list.append(
                OBTRoomMajorCategory(
                    object_index=id,
        ...
        for platform_name, platform in self.scene_graph_tree.platforms.items():
            ...
            major_category_list, minor_category_list, single_object_list = platform.get_categories_and_objects()
        ...
        for node_name, node in self.scene_graph_tree.nodes.items():
            if node.is_multilayer_object():
                major_category_list, minor_category_list = node.get_categories_and_objects_for_mlo()
        ...
        return platform_list, multilayer_object_list, room_major_category_list, room_minor_category_list 
        pass
    
    pass 

def main():
    print("--- Comprehensive Task Pattern Parsing Tests ---")

    with open("pattern.txt", "r", encoding="utf-8") as file:
        task_patterns = file.readlines()
    ...
    for i, pattern in enumerate(task_patterns):
        ...
        print(f"  Multilayer Object Info: {task_instance.require_multilayer_object_info}")
