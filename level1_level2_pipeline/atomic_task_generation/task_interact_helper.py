...
class TaskInteractHelper:
    ...
    def __handle_ambiguous_item(self):
        """
        solve
        """
        if not self.task.is_ambiguous(self.scene_graph_tree):
            return
        ...
        if self.task.item.is_ambiguous:
            object_to_show = self.task.item
            save_path = f"{self.image_path}/image4interact/{self.model}/Task_{self.task_id}_AmbiguousObject.png"
    ...
    def apply_action(self, state=None):
        ...
        if not self.status and self.generate_mistake_note:
            self.scene_graph_tree = self.initial_scene_graph_tree
            self.scene_graph_tree.corresponding_scene = self.scene
            ...
            if len(goal_action_list) != len(goal_explanation_list) or len(
                goal_action_list
            ) != len(goal_picture_list):
                import ipdb 
                ipdb.set_trace()
            ...
            def write_reflection_to_file(
                reflection_conversation_list,
                filename="mistake_note.txt",
            ):

                try:
                    with open(filename, "a", encoding="utf-8") as f:
                        # 使用json.dumps将整个列表转换为单行JSON字符串
                        json_str = json.dumps(
