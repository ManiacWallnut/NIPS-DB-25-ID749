...
REPLICA_DATASET_ROOT_PATH = Path(
    "/mnt/windows_e/workplace/task_generation/replica_dataset"
)
...
def parse_replica(input_json_path, output_json_path):
    ...
    if "_0" in input_json_path:
        not_desired_objects.extend(
            ["frl_apartment_camera_02", "frl_apartment_tv_object"]
        )
...
def main():
    ...
    parser.add_argument(
        "--input_json_path",
        type=str,
        default=f"{REPLICA_DATASET_ROOT_PATH}/configs/scenes/apt_0.scene_instance.json",
    )
