...
OBJECT_ROOT_PATH = os.path.join(DATASET_ROOT_PATH, "assets", "ai2thor", "models")
...
def get_object_config_path(template_name):
    ...
    object_config_path = os.path.join(
        DATASET_ROOT_PATH, "configs", template_name + ".object_config.json"
    )
...
def get_scene_glb_path(template_name):
    ...
    """
    scene_glb_path = os.path.join(DATASET_ROOT_PATH, "assets", template_name + ".glb")
...
def parse_ai2thor(input_scene_json_path, output_scene_json_path):
    ...
    for obj in scene_data["object_instances"]:
        ...
        rotation_xyz = np.dot(rotation_matrix, rotation[-3:])
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.path.join(DATASET_ROOT_PATH, "parsed_scene.json"),
